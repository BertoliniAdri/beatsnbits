...
// renvoie un bouton à partir de la lettre (l'intitulé ou k pour key) et l'array ou il se trouve (table)
//function get_button_from_letter_time(k, t, table)
//{
//    for (var i = 0; i < table.length; i++) {
//        if (table[i].time === t && table[i].key === k) {
//            return table[i];
//        }
//    }
//}
...
//            get_button_from_letter_time(key_time[i][1], Math.round(percentage_to_time(key_time[i][0])), sound_markers).activate(80);
...
function create_audioCategories_buttons(categorie)
{
    ...
    for (var i = 0, len = soundFiles.length; i < len; i++)
    {
        ...
        {
            soundFiles_buttons.push(new Button("sample " + (i < 10 ? "0" + (i + 1) : (i + 1)), "librarySamples_1", soundFiles[i][0], false));
        }
    }
